...
This capability no longer produces a score of its own. Its findings feed **Unfinished Risk** (formerly the AI Slop Score), a composite meter computed by capybari-core (0 = clean, 100 = unreviewed and unfinished). For websites it also publishes `site-depth` evidence (signs of effort: content pages, specific figures and names, finished metadata, trust pages, extra craft), which capybari-core turns into the **Looks Shipped** score (score ID build-depth; 0 = thin, 100 = looks shipped). See the [scoring methodology](https://github.com/capybari-repo/capybari-docs/blob/main/methodology/scoring.md#ai-slop).
